...
app.get('/CaptchaConfig', async (req, res) => {
    try {
        const apiUrl = `https://www.indianrail.gov.in/enquiry/CaptchaConfig?_=${new Date().getTime()}`; // Example API URL
        const response = await axios.get(apiUrl);
        if (response.data != 0) {
            const res1 = await axios.get(`https://www.indianrail.gov.in/enquiry/captchaDraw.png?${new Date().getTime()}`);
            res.json(res1.data);
        } else {
...
app.get('/CaptchaDraw', async (req, res) => {
    try {
...
app.get('/fetch-captcha', async (req, res) => {
    try {
...
app.get('/get-train-live-status', async (req, res) => {
    try {
        const { data: html } = await axios.get(`https://www.railyatri.in/live-train-status/${req.query.trainNo}`);
...
app.get('/v2/get-train-live-status', async (req, res) => {
    try {
        const response = await axios.get(`https://www.redbus.in/railways/api/getLtsDetails?trainNo=${req.query.trainNo}`);
        res.json(response.data)
    } catch (error) {
        res.json(error)
    }
...
app.get('/get-train-time-table', async (req, res) => {
    try {
        const { data: html } = await axios.get(`https://www.railyatri.in/time-table/${req.query.trainNo}`);
        const $ = cheerio.load(html);
        const output = $('#__NEXT_DATA__').text();
        res.json(JSON.parse(output))
    } catch (error) {
...
app.get('/get-pnr-status', async (req, res) => {
    try {
        ...
        const response = await axios.get('https://www.redbus.in/railways/api/getPnrData', { params });
        res.json(response.data);
    } catch (error) {
...
app.get('/get-captcha-value', async (req, res) => {
    try {
...
app.get('/get-train-between-station', async (req, res) => {
    try {
...
app.get('/get-next-days-seat-availability', async (req, res) => {
    try {
        // console.log(req.query);
        const params = new URLSearchParams();
